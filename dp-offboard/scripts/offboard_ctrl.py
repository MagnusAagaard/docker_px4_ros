...
"""
offboard_ctrl.py: Controlling the setpoints, eg. used for testing the OptiTrack system
...
class OffboardControl:
    ...
    *
    ...
    def setpoint_controller_server(self):
        #s_start = rospy.Service('setpoint_controller/run', Empty, self.run)
        ...
        #s_takeoff = rospy.Service('setpoint_controller/takeoff_setpoint', Empty, self.takeoff_setpoint)
        s_circle = rospy.Service('setpoint_controller/circle', Empty, self.start_circle)
        s_sphere = rospy.Service('setpoint_controller/sphere', Empty, self.start_sphere)
        s_up_n_down = rospy.Service('setpoint_controller/up_n_down', Empty, self.start_up_n_down)
    ...
    * set_target:
    ...
    Up and Down
    """
    def start_up_n_down(self, r):
        self.t_up_n_down = threading.Thread(target=self.up_n_down)
        self.t_up_n_down.start()
        print(">> Starting Up-and-Dowm (Thread)")
    ...
    def up_n_down(self):
        ...
        if(self.state != "RUNNING"):
            ...
            self.set_state("UPNDOWM")
            switch = False
            while self.state == "UPNDOWM":
                if(switch == True):
                    z = upper_limit
                    switch = False
                else:
    ...
    Sphere
    """
    def calc_sphere(self, phi, costheta):
    ...
    def start_sphere(self, r):
        self.t_sphere = threading.Thread(target=self.sphere)
        self.t_sphere.start()
        print(">> Starting sphere (Thread)")
    ...
    def sphere(self):
        ...
        if(self.state != "RUNNING"):
            ...
            self.set_state("SPHERE")

            while self.state == "SPHERE":
                for i in np.arange(-1,1,0.05):
                    for j in np.arange(0,np.pi*2,0.05):
                        tmp = self.calc_sphere(j,i)

                        #print("Set: {}  {} --> {}".format(j,i,tmp))

                        x = scale * tmp[0]
                        y = scale * tmp[1]
                        z = scale * tmp[2] + offset
